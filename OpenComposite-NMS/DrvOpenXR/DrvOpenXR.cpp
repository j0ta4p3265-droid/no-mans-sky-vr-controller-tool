...
IBackend* DrvOpenXR::CreateOpenXRBackend()
{
	...
	// Some deployed runtimes (notably VDXR) reject the very new 1.1 header
	// version used to build this compatibility fork. OpenComposite only relies
	// on OpenXR 1.0 core entry points, so negotiate the oldest compatible core.
	appInfo.apiVersion = XR_MAKE_VERSION(1, 0, 0);
	OOVR_LOG("Requesting OpenXR core API 1.0.0");
	...
	for (const char* extension : extensions)
		OOVR_LOGF("Enabling extension: %s", extension);
