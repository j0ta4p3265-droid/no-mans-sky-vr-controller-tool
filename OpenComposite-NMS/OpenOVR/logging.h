...
inline const char* oovr_source_filename(const char* path)
{
...
#define OOVR_LOG(msg) oovr_log_raw(oovr_source_filename(__FILE__), __LINE__, __FUNCTION__, msg)
#define OOVR_LOGF(...) oovr_log_raw_format(oovr_source_filename(__FILE__), __LINE__, __FUNCTION__, __VA_ARGS__)
...
#define OOVR_ABORT(msg)                                        \
	do {                                                       \
		oovr_abort_raw(oovr_source_filename(__FILE__), __LINE__, __FUNCTION__, msg); \
	} while (0)
#define OOVR_ABORT_T(msg, title)                                      \
	do {                                                              \
		oovr_abort_raw(oovr_source_filename(__FILE__), __LINE__, __FUNCTION__, msg, title); \
	} while (0)
#define OOVR_ABORTF(msg, ...)                                                        \
	do {                                                                             \
		oovr_abort_raw(oovr_source_filename(__FILE__), __LINE__, __FUNCTION__, msg, nullptr, __VA_ARGS__); \
	} while (0)
...
#define OOVR_SOFT_ABORTF(msg, ...)                                                                      \
	do {                                                                                                \
		...
		oovr_soft_abort_raw(oovr_source_filename(__FILE__), __LINE__, __FUNCTION__, &soft_abort_hit_count, msg, __VA_ARGS__); \
	} while (0)
	...
	do {                                                                              \
		...
		if (failed_vk_abort_result < VK_SUCCESS) {                                    \
			OOVR_ABORTF("Vulkan Call failed, aborting. %s:%d %s. Error code: %d\n%s", \
			    oovr_source_filename(__FILE__), __LINE__, __func__, failed_vk_abort_result, #expression);   \
		}                                                                             \
	} while (0)
		...
		if (XR_FAILED(failed_xr_abort_result)) {                                          \
			if (xr_instance != XR_NULL_HANDLE) {                                          \
				...
				OOVR_ABORTF("OpenXR Call failed, aborting. %s:%d %s. Error code: %s\n%s", \
				    oovr_source_filename(__FILE__), __LINE__, __func__, buf, #expression);                      \
			} else {                                                                      \
				OOVR_ABORTF("OpenXR Call failed, aborting. %s:%d %s. Error code: %d\n%s", \
				    oovr_source_filename(__FILE__), __LINE__, __func__, failed_xr_abort_result, #expression);   \
			}                                                                             \
		}                                                                                 \
	} while (0)
