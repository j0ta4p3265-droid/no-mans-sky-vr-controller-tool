...
{
	std::string name = pchRenderModelName ? pchRenderModelName : "";
	if (name.find("oculus_quest2_controller_") != std::string::npos || name == "renderLeftHand" || name == "renderRightHand")
		return 8;
    ...
    char* pchComponentName, uint32_t unComponentNameLen)
{
	...
	const bool left = name.find("left") != std::string::npos || name == "renderLeftHand";
	const char* leftComponents[] = { "body", "trigger", "button_x", "button_y", "thumbstick", "button_grip", "button_enter", "base" };
	const char* rightComponents[] = { "body", "trigger", "button_a", "button_b", "thumbstick", "button_grip", "button_enter", "base" };
	if (unComponentIndex >= 8) return 0;
	std::string component = left ? leftComponents[unComponentIndex] : rightComponents[unComponentIndex];

	if (pchComponentName) {
		// +1 for NULL
		if (unComponentNameLen < component.length() + 1) {
			OOVR_ABORT("unComponentNameLen too small!");
		}

		// TODO should we allow too small buffers?
		strcpy_s(pchComponentName, unComponentNameLen, component.c_str());
		pchComponentName[unComponentNameLen - 1] = 0;
	}

	// +1 for null
	return (uint32_t)component.length() + 1;
}

uint64_t BaseRenderModels::GetComponentButtonMask(const char* pchRenderModelName, const char* pchComponentName)
{
	std::string component = pchComponentName ? pchComponentName : "";
	if (component == "trigger") return vr::ButtonMaskFromId(vr::k_EButton_SteamVR_Trigger);
	if (component == "button_grip") return vr::ButtonMaskFromId(vr::k_EButton_Grip);
	if (component == "thumbstick") return vr::ButtonMaskFromId(vr::k_EButton_SteamVR_Touchpad);
	if (component == "button_a" || component == "button_x") return vr::ButtonMaskFromId(vr::k_EButton_A);
	if (component == "button_b" || component == "button_y") return vr::ButtonMaskFromId(vr::k_EButton_ApplicationMenu);
	if (component == "button_enter") return vr::ButtonMaskFromId(vr::k_EButton_System);
	return 0;
}
...
bool BaseRenderModels::GetComponentState(const char* pchRenderModelName, const char* pchComponentName,
    ...
    OOVR_RenderModel_ComponentState_t* pComponentState)
{
	static std::set<std::string> loggedNmsComponentCalls;
	const std::string diagnosticKey = std::string(pchRenderModelName ? pchRenderModelName : "<null>") + " | "
	    + (pchComponentName ? pchComponentName : "<null>");
	if (loggedNmsComponentCalls.insert(diagnosticKey).second)
		OOVR_LOGF("NMS hand diagnostic: render model/component = %s", diagnosticKey.c_str());
	...
	if (pchRenderModelName) {
		...
		} else if (renderModelName == "oculus_quest2_controller_left") {
			hand = ITrackedDevice::HAND_LEFT;
		} else if (renderModelName == "oculus_quest2_controller_right") {
			hand = ITrackedDevice::HAND_RIGHT;
		} else {
