    ...
    uint32_t nNumCoordinates, const DistortionCoordinate_t* pInput, DistortionCoordinate_t* pOutput)
{
	if (!pInput || !pOutput)
		return false;
	if (eChannel == EVRDistortionChannel::InverseRed || eChannel == EVRDistortionChannel::InverseGreen
	    || eChannel == EVRDistortionChannel::InverseBlue || eChannel == EVRDistortionChannel::Count)
		return false;

	for (uint32_t i = 0; i < nNumCoordinates; ++i) {
		DistortionCoordinates_t distortion{};
		if (!ComputeDistortion(eEye, pInput[i].u, pInput[i].v, &distortion))
			return false;

		const float* channel = distortion.rfRed;
		if (eChannel == EVRDistortionChannel::Green)
			channel = distortion.rfGreen;
		else if (eChannel == EVRDistortionChannel::Blue)
			channel = distortion.rfBlue;

		pOutput[i].u = bAsNormalizedDeviceCoordinates ? channel[0] * 2.0f - 1.0f : channel[0];
		pOutput[i].v = bAsNormalizedDeviceCoordinates ? channel[1] * 2.0f - 1.0f : channel[1];
	}
	return true;
}
...
bool BaseSystem::PollNextEventWithPoseAndOverlays(ETrackingUniverseOrigin eOrigin, VREvent_t* pEvent, uint32_t uncbVREvent,
    vr::TrackedDevicePose_t* pTrackedDevicePose, VROverlayHandle_t* pulOverlayHandle)
{
	if (pulOverlayHandle)
