...
static int NmsRecenterPart(const std::string& actionName)
{
	for (int part = 1; part <= 2; ++part) {
		const std::string suffix = "/in/vr_recentre" + std::to_string(part);
		if (actionName.size() >= suffix.size()
		    && actionName.compare(actionName.size() - suffix.size(), suffix.size(), suffix) == 0)
			return part;
	}
	return 0;
}
...
template <typename T>
std::string BaseInput::Registry<T>::LookupName(RegHandle handle) const
{
	auto iter = namesByHandle.find(handle);
	if (iter == namesByHandle.end())
		return {};
	return lowerStr(iter->second);
}
	...
	for (std::string setFullName : bindingsJson.getMemberNames()) {
		...
		for (const auto& srcJson : setJson["sources"]) {
			...
			for (const std::string& inputName : inputsJson.getMemberNames()) {
				...
				ActionSource source;
				source.svrPathName = importBasePath;
				source.svrDevicePathName = importBasePath.substr(0, importBasePath.find("/input/"));
				source.svrInputPathName = importBasePath.substr(importBasePath.find("/input/"));
				source.modeName = srcJson["mode"].asString();
				source.slot = inputName;
				// NMS is running as an Oculus Touch application through VDXR. Its
				// glyph lookup must see a single coherent controller binding table.
				// Mixing Knuckles/Vive/WMR sources (loaded before TOUCH.JSON) makes
				// NMS reject the table and display the yellow warning triangle.
				if (profile.GetPath() == "/interaction_profiles/oculus/touch_controller")
					action->bindingSources.push_back(source);
...
void BaseInput::CreateLegacyActions()
{
	...
	for (int i = 0; i < 2; i++) {
		...
		create(&ctrl.thumbrestTouch, "thumbrest-touch", "Capacitive Thumbrest (Touch)", XR_ACTION_TYPE_BOOLEAN_INPUT);
...
EVRInputError BaseInput::GetActionHandle(const char* pchActionName, VRActionHandle_t* pHandle)
{
	*pHandle = actions.LookupHandle(pchActionName);
	if (!cast_AH(*pHandle)) {
		const std::string requestedName = lowerStr(pchActionName);
		// NMS still requests these legacy analogue ship actions even though they
		// are absent from its current manifest. Keep their stable dummy handles
		// valid; GetAnalogActionData supplies them from the right Touch stick.
		if (requestedName == "/actions/shipcontrols/in/ship_yaw"
		    || requestedName == "/actions/shipcontrols/in/ship_pitch")
			return vr::VRInputError_None;
		OOVR_LOGF("Action name not found: %s", pchActionName);
...
void BaseInput::UpdateThumbrestTripleTap(int hand, bool raw, bool active, const char* handName)
{
	auto& detector = thumbrestTripleTap[hand];
	if (detector.syncSerial == syncSerial)
		return;
	const int64_t nowMs = std::chrono::duration_cast<std::chrono::milliseconds>(
	    std::chrono::steady_clock::now().time_since_epoch())
	                          .count();
	bool output = detector.outputUntilMs >= 0 && nowMs < detector.outputUntilMs;

	if (raw != detector.previousRaw) {
		OOVR_LOGF("NMS %s thumbrest raw transition: %d (active=%d)", handName, raw ? 1 : 0, active ? 1 : 0);
	}
	if (raw && !detector.previousRaw) {
		const int windowMs = std::max(100, oovr_global_configuration.ThumbrestDoubleTapWindowMs());
		if (detector.lastTapMs < 0 || nowMs - detector.lastTapMs > windowMs)
			detector.tapCount = 1;
		else
			detector.tapCount++;
		detector.lastTapMs = nowMs;
		OOVR_LOGF("NMS %s thumbrest tap %d/3", handName, detector.tapCount);
		if (detector.tapCount >= 3) {
			output = true;
			detector.outputUntilMs = nowMs + 1200;
			detector.tapCount = 0;
			detector.lastTapMs = -1;
			OOVR_LOGF("NMS %s thumbrest triple-tap: holding LS + RS for 1200 ms", handName);
		}
	}
	if (!output && detector.outputUntilMs >= 0)
		detector.outputUntilMs = -1;

	detector.output = output;
	detector.changed = output != detector.previousOutput;
	detector.previousOutput = output;
	detector.previousRaw = raw;
	detector.syncSerial = syncSerial;
	OOVR_LOG_ONCE("NMS independent thumbrest triple-tap detectors enabled");
}
    ...
    VRInputValueHandle_t ulRestrictToDevice)
{
	...
	const int recenterPart = NmsRecenterPart(act->fullName);
	const bool thumbrestEnabled[2] = {
	    oovr_global_configuration.EnableThumbrestDoubleTap(),
	    oovr_global_configuration.EnableRightThumbrestDoubleTap()
	};
	const char* thumbrestHandPath[2] = { "/user/hand/left", "/user/hand/right" };
	bool usesThumbrest[2] = { false, false };
	for (int hand = 0; hand < 2; ++hand) {
		usesThumbrest[hand] = std::any_of(act->bindingSources.begin(), act->bindingSources.end(), [&](const ActionSource& source) {
			return source.svrDevicePathName == thumbrestHandPath[hand]
			    && source.svrInputPathName == "/input/thumbrest"
			    && source.slot == "touch";
		});
	}
	...
	for (int i = 0; i < allSubactionPaths.size(); i++) {
		...
		if (recenterPart != 0 && i < 2 && thumbrestEnabled[i] && usesThumbrest[i]) {
				UpdateThumbrestTripleTap(
				    i,
				    state.currentState == XR_TRUE,
				    state.isActive == XR_TRUE,
				    i == ITrackedDevice::HAND_RIGHT ? "right" : "left");
				state.currentState = thumbrestTripleTap[i].output ? XR_TRUE : XR_FALSE;
				state.changedSinceLastSync = thumbrestTripleTap[i].changed ? XR_TRUE : XR_FALSE;
				if (thumbrestTripleTap[i].output)
					state.isActive = XR_TRUE;
		} else if (recenterPart != 0 && i < 2 && usesThumbrest[i]) {
				state.currentState = XR_FALSE;
				state.changedSinceLastSync = XR_FALSE;
		}
	...
	const bool macroOutput = (thumbrestEnabled[0] && usesThumbrest[0] && thumbrestTripleTap[0].output)
	    || (thumbrestEnabled[1] && usesThumbrest[1] && thumbrestTripleTap[1].output);
	if (recenterPart != 0 && macroOutput) {
		const char* iconPath = recenterPart == 1
		    ? "/user/hand/left/input/joystick/click"
		    : "/user/hand/right/input/joystick/click";
		OOVR_FALSE_ABORT(GetInputSourceHandle(iconPath, &pActionData->activeOrigin) == vr::VRInputError_None);
	}
	...
	if (!NmsVulkanKeyboard::IsActive()
	    && (act->fullName == "/actions/shipcontrols/in/right" || act->fullName == "/actions/shipcontrols/in/left")) {
		XrActionStateGetInfo axisInfo{ XR_TYPE_ACTION_STATE_GET_INFO };
		axisInfo.action = legacyControllers[ITrackedDevice::HAND_RIGHT].stickX;
		XrActionStateFloat axis{ XR_TYPE_ACTION_STATE_FLOAT };
		OOVR_FAILED_XR_ABORT(xrGetActionStateFloat(xr_session.get(), &axisInfo, &axis));
		const bool wantsRight = act->fullName == "/actions/shipcontrols/in/right";
		const bool pressed = axis.isActive && (wantsRight ? axis.currentState > 0.5f : axis.currentState < -0.5f);
		pActionData->bActive = axis.isActive;
		pActionData->bChanged = pActionData->bState != pressed;
		pActionData->bState = pressed;
	}
	if (NmsVulkanKeyboard::IsActive()) {
    ...
    VRInputValueHandle_t ulRestrictToDevice)
{
	...
	if (!act) {
		// Recent NMS still asks for this older analogue action even though its
		// shipped manifest only contains the newer left/right D-pad actions.
		// SteamVR supplies the right Touch stick X axis for it; reproduce that
		// compatibility behaviour instead of rejecting the pre-manifest handle.
		const std::string virtualActionName = actions.LookupName(action);
		if (virtualActionName == "/actions/shipcontrols/in/ship_yaw"
		    || virtualActionName == "/actions/shipcontrols/in/ship_pitch") {
			XrActionStateGetInfo axisInfo{ XR_TYPE_ACTION_STATE_GET_INFO };
			axisInfo.action = virtualActionName == "/actions/shipcontrols/in/ship_yaw"
			    ? legacyControllers[ITrackedDevice::HAND_RIGHT].stickX
			    : legacyControllers[ITrackedDevice::HAND_RIGHT].stickY;
			XrActionStateFloat axis{ XR_TYPE_ACTION_STATE_FLOAT };
			OOVR_FAILED_XR_ABORT(xrGetActionStateFloat(xr_session.get(), &axisInfo, &axis));
			pActionData->x = axis.currentState;
			pActionData->deltaX = axis.changedSinceLastSync ? axis.currentState : 0.0f;
			pActionData->bActive = axis.isActive;
			return VRInputError_None;
		}
		OOVR_LOG_ONCE("WARNING: Invalid action handle passed!");
	...
	for (int i = 0; i < allSubactionPaths.size(); i++) {
		...
		case ActionType::Vector1: {
			...
			pActionData->y = 0;
			pActionData->z = 0;
			auto& history = act->analogHistory.at(i);
			if (history.syncSerial != syncSerial) {
				history.deltaX = state.currentState - history.previousX;
				history.deltaY = -history.previousY;
				history.deltaZ = -history.previousZ;
				history.previousX = state.currentState;
				history.previousY = 0.0f;
				history.previousZ = 0.0f;
				history.syncSerial = syncSerial;
			}
			pActionData->deltaX = history.deltaX;
			pActionData->deltaY = history.deltaY;
			pActionData->deltaZ = history.deltaZ;
			pActionData->bActive = state.isActive;
			pActionData->activeOrigin = activeOriginFromSubaction(act, allSubactionPathNames[i].c_str());
			break;
		}
		case ActionType::Vector2: {
			...
			// SteamVR applies joystick deadzones radially and expands the remaining
			// travel back to the full range. Reproduce that behaviour for OpenXR.
			if (deadZoneSize > 0.0f) {
				float magnitude = sqrtf(state.currentState.x * state.currentState.x + state.currentState.y * state.currentState.y);
				if (magnitude <= deadZoneSize) {
					state.currentState.x = 0.0f;
					state.currentState.y = 0.0f;
				} else if (deadZoneSize < 1.0f && magnitude > 0.0f) {
					float remappedMagnitude = (magnitude - deadZoneSize) / (1.0f - deadZoneSize);
			...
			if (i == 1) {
				float scale = std::max(0.0f, oovr_global_configuration.RightJoystickScale());
				state.currentState.x *= scale;
				state.currentState.y *= scale;
			}
			// OpenVR analogue axes are bounded. In particular, custom scales above
			// one must saturate rather than feeding over-range values to the game.
			float outputMagnitude = sqrtf(state.currentState.x * state.currentState.x + state.currentState.y * state.currentState.y);
			if (outputMagnitude > 1.0f) {
				state.currentState.x /= outputMagnitude;
				state.currentState.y /= outputMagnitude;
			}
			...
			auto& history = act->analogHistory.at(i);
			if (history.syncSerial != syncSerial) {
				history.deltaX = state.currentState.x - history.previousX;
				history.deltaY = state.currentState.y - history.previousY;
				history.deltaZ = -history.previousZ;
				history.previousX = state.currentState.x;
				history.previousY = state.currentState.y;
				history.previousZ = 0.0f;
				history.syncSerial = syncSerial;
			}
			pActionData->deltaX = history.deltaX;
    ...
    InputPoseActionData_t* pActionData, uint32_t unActionDataSize, VRInputValueHandle_t ulRestrictToDevice)
{
	...
	if (act->type == ActionType::Skeleton) {
		OOVR_LOG_ONCE("NMS hand route: GetPoseActionData is using the SKELETON pose branch");
	...
	if (act->type != ActionType::Pose)
		OOVR_ABORTF("Invalid action type %d for action %s", act->type, act->fullName.c_str());
	static std::set<std::string> loggedNmsPoseActions;
	if (loggedNmsPoseActions.insert(act->fullName).second)
		OOVR_LOGF("NMS hand route: regular POSE action requested: %s", act->fullName.c_str());
...
EVRInputError BaseInput::GetEyeTrackingDataRelativeToNow(VRActionHandle_t action, ETrackingUniverseOrigin eOrigin, float fPredictedSecondsFromNow, vr::VREyeTrackingData_t* pEyeTrackingData, uint32_t ulEyeTrackingDataSize)
{
	if (!pEyeTrackingData || ulEyeTrackingDataSize < sizeof(vr::VREyeTrackingData_t)) {
		return VRInputError_BufferTooSmall;
	}
	*pEyeTrackingData = {};
	return VRInputError_None;
}
EVRInputError BaseInput::GetEyeTrackingDataForNextFrame(VRActionHandle_t action, ETrackingUniverseOrigin eOrigin, vr::VREyeTrackingData_t* pEyeTrackingData, uint32_t ulEyeTrackingDataSize)
{
	return GetEyeTrackingDataRelativeToNow(action, eOrigin, 0.0f, pEyeTrackingData, ulEyeTrackingDataSize);
}
    ...
    EVRSkeletalMotionRange eMotionRange, VR_ARRAY_COUNT(unTransformArrayCount) VRBoneTransform_t* pTransformArray, uint32_t unTransformArrayCount)
{
	...
	GET_ACTION_FROM_HANDLE(action, actionHandle);
	static std::set<std::string> loggedNmsSkeletonCalls;
	const std::string skeletonDiagnostic = std::to_string((int)action->skeletalHand) + "/"
	    + std::to_string((int)eTransformSpace) + "/" + std::to_string((int)eMotionRange);
	if (loggedNmsSkeletonCalls.insert(skeletonDiagnostic).second)
		OOVR_LOGF("NMS hand route: GetSkeletalBoneData hand/space/range = %s", skeletonDiagnostic.c_str());
    ...
    VR_ARRAY_COUNT(originOutCount) VRInputValueHandle_t* originsOut, uint32_t originOutCount)
{
	...
	// The experimental thumbrest macro has no NMS glyph. Keep reporting the
	// original two-stick chord while the actual OpenXR bindings read the sensor.
	const int recenterPart = NmsRecenterPart(act->fullName);
	if ((oovr_global_configuration.EnableThumbrestDoubleTap()
	        || oovr_global_configuration.EnableRightThumbrestDoubleTap())
	    && recenterPart != 0) {
		if (originOutCount == 0)
		...
		const char* iconPath = recenterPart == 1
		    ? "/user/hand/left/input/joystick/click"
		    : "/user/hand/right/input/joystick/click";
		OOVR_FALSE_ABORT(GetInputSourceHandle(iconPath, &originsOut[0]) == vr::VRInputError_None);
		return VRInputError_None;
	}

	// NMS queries origins while building its controller glyph table. Return the
	// full physical paths from the loaded binding JSON; returning only the hand
	// loses whether the origin was A/B, trigger, grip or thumbstick.
	if (!act->bindingSources.empty()) {
		uint32_t count = std::min<uint32_t>((uint32_t)act->bindingSources.size(), originOutCount);
		for (uint32_t i = 0; i < count; ++i) {
			const ActionSource& source = act->bindingSources[i];
			std::string path = source.svrPathName;
			if (!source.slot.empty() && source.slot != "position" && source.slot != "east" && source.slot != "west" && source.slot != "north" && source.slot != "south")
				path += "/" + source.slot;
			OOVR_FALSE_ABORT(GetInputSourceHandle(path.c_str(), &originsOut[i]) == vr::VRInputError_None);
		}
		return act->bindingSources.size() > originOutCount ? VRInputError_MaxCapacityReached : VRInputError_None;
	}
	...
	// Preserve full source paths so callers can identify the physical component.
...
EVRInputError BaseInput::GetOriginTrackedDeviceInfo(VRInputValueHandle_t origin, InputOriginInfo_t* info, uint32_t unOriginInfoSize)
{
	...
	const InputValueHandle* input = cast_IVH(origin);
	const std::string& path = input->path;
	const char* component = "body";
	if (path.find("/input/a/") != std::string::npos) component = "button_a";
	else if (path.find("/input/b/") != std::string::npos) component = "button_b";
	else if (path.find("/input/x/") != std::string::npos) component = "button_x";
	else if (path.find("/input/y/") != std::string::npos) component = "button_y";
	else if (path.find("/input/joystick") != std::string::npos || path.find("/input/thumbstick") != std::string::npos) component = "thumbstick";
	else if (path.find("/input/trigger") != std::string::npos) component = "trigger";
	else if (path.find("/input/grip") != std::string::npos || path.find("/input/squeeze") != std::string::npos) component = "button_grip";
	else if (path.find("/input/menu") != std::string::npos || path.find("/input/system") != std::string::npos) component = "button_enter";
	strcpy_arr(info->rchRenderModelComponentName, component);
    ...
    uint32_t unBindingInfoSize, uint32_t unBindingInfoCount, uint32_t* punReturnedBindingInfoCount)
{
	...
	const int recenterPart = NmsRecenterPart(action->fullName);
	if ((oovr_global_configuration.EnableThumbrestDoubleTap()
	        || oovr_global_configuration.EnableRightThumbrestDoubleTap())
	    && recenterPart != 0) {
		if (punReturnedBindingInfoCount)
			*punReturnedBindingInfoCount = 1;
		if (unBindingInfoCount == 0)
			return VRInputError_MaxCapacityReached;
		OOVR_InputBindingInfo_t& info = bindingInfo[0];
		strcpy_arr(info.rchDevicePathName,
		    recenterPart == 1 ? "/user/hand/left" : "/user/hand/right");
		strcpy_arr(info.rchInputPathName, "/input/joystick");
		strcpy_arr(info.rchModeName, "joystick");
		strcpy_arr(info.rchSlotName, "click");
		strcpy_arr(info.rchInputSourceType, "joystick");
		return VRInputError_None;
	}

	// The application may ask before OpenXR has selected an interaction profile.
	// SteamVR can still answer from the binding JSON, so do the same here.
	if (!action->bindingSources.empty()) {
		uint32_t count = std::min<uint32_t>((uint32_t)action->bindingSources.size(), unBindingInfoCount);
		if (punReturnedBindingInfoCount) *punReturnedBindingInfoCount = count;
		for (uint32_t i = 0; i < count; ++i) {
			const ActionSource& source = action->bindingSources[i];
			OOVR_InputBindingInfo_t& info = bindingInfo[i];
			strcpy_arr(info.rchDevicePathName, source.svrDevicePathName.c_str());
			strcpy_arr(info.rchInputPathName, source.svrInputPathName.c_str());
			strcpy_arr(info.rchModeName, source.modeName.c_str());
			strcpy_arr(info.rchSlotName, source.slot.c_str());
			const bool trigger = source.svrInputPathName.find("trigger") != std::string::npos || source.svrInputPathName.find("grip") != std::string::npos || source.svrInputPathName.find("squeeze") != std::string::npos;
			const bool joystick = source.svrInputPathName.find("joystick") != std::string::npos || source.svrInputPathName.find("thumbstick") != std::string::npos;
			strcpy_arr(info.rchInputSourceType, trigger ? "trigger" : (joystick ? "joystick" : "button"));
		}
		return VRInputError_None;
	}
	...
	for (int i = 0; i < count; i++) {
		...
		// SteamVR reports the physical source type independently from the action
		// type. NMS uses this to choose controller glyphs (notably triggers).
		const bool isTrigger = pathStr.find("/input/trigger") != std::string::npos || pathStr.find("/input/squeeze") != std::string::npos || pathStr.find("/input/grip") != std::string::npos;
		const bool isJoystick = pathStr.find("/input/joystick") != std::string::npos || pathStr.find("/input/thumbstick") != std::string::npos;
		switch (action->type) {
		case ActionType::Boolean:
			strcpy_arr(info.rchModeName, "button");
			strcpy_arr(info.rchInputSourceType, isTrigger ? "trigger" : (isJoystick ? "joystick" : "button"));
			break;
		case ActionType::Vector1:
			strcpy_arr(info.rchModeName, "trigger");
			strcpy_arr(info.rchInputSourceType, "trigger");
			break;
		case ActionType::Vector2:
			strcpy_arr(info.rchModeName, "joystick");
			strcpy_arr(info.rchInputSourceType, "joystick");
...
bool BaseInput::GetLegacyControllerState(vr::TrackedDeviceIndex_t controllerDeviceIndex, vr::VRControllerState_t* state)
{
	*state = {};
	...
	if (deadZoneSize > 0.0f) {
		float magnitude = sqrtf(thumbstick.x * thumbstick.x + thumbstick.y * thumbstick.y);
		if (magnitude <= deadZoneSize) {
			thumbstick.x = 0.0f;
			thumbstick.y = 0.0f;
		} else if (deadZoneSize < 1.0f && magnitude > 0.0f) {
	...
	float outputMagnitude = sqrtf(thumbstick.x * thumbstick.x + thumbstick.y * thumbstick.y);
	if (outputMagnitude > 1.0f) {
		thumbstick.x /= outputMagnitude;
		thumbstick.y /= outputMagnitude;
	}
	if (std::abs(thumbstick.x) <= deadZoneXSize) thumbstick.x = 0.0f;
	if (std::abs(thumbstick.y) <= deadZoneYSize) thumbstick.y = 0.0f;
	...
	// Touch/VDXR exposes squeeze primarily as an analogue value. Some runtimes
	// do not activate the boolean action converted from /squeeze/value, so use
	// the physical squeeze value as the reliable click source as SteamVR does.
	if (grip.x >= 0.55f || (inputSmoothingEnabled && smoothInput.getSmoothedGripClickValue(hand)) || (!inputSmoothingEnabled && readBool(ctrl.gripClick))) {
