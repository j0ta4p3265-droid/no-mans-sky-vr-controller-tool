...
public:
	...
	virtual EVRInputError GetEyeTrackingDataRelativeToNow(VRActionHandle_t action, ETrackingUniverseOrigin eOrigin, float fPredictedSecondsFromNow, vr::VREyeTrackingData_t* pEyeTrackingData, uint32_t ulEyeTrackingDataSize);
	virtual EVRInputError GetEyeTrackingDataForNextFrame(VRActionHandle_t action, ETrackingUniverseOrigin eOrigin, vr::VREyeTrackingData_t* pEyeTrackingData, uint32_t ulEyeTrackingDataSize);
...
private:
	...
	struct DpadBindingInfo {
		...
		// SteamVR's default D-pad deadzone is 25% when the binding does not
		// provide an explicit deadzone_pct parameter (as in NMS's Touch binding).
		static constexpr float dpadDeadzoneRadius = 0.25f;
	...
	public:
		...
		// OpenVR defines analogue deltas as the change since the preceding
		// UpdateActionState call. It also guarantees repeated reads between two
		// updates return the same result. Keep independent history per hand so a
		// restricted and an unrestricted read cannot consume each other's delta.
		struct AnalogHistory {
			...
			float deltaY = 0.0f;
			float deltaZ = 0.0f;
			uint64_t syncSerial = static_cast<uint64_t>(-1);
		};
		std::array<AnalogHistory, 2> analogHistory;
		...
		std::vector<ActionSource> bindingSources;
	...
	public:
		...
		std::string LookupName(RegHandle handle) const;
	...
	// The NMS convenience gesture is read from a dedicated legacy OpenXR action,
	// keeping the game's normal stick-click recenter bindings untouched.
	struct ThumbrestTripleTapState {
		bool previousRaw = false;
		bool previousOutput = false;
		bool output = false;
		bool changed = false;
		int tapCount = 0;
		int64_t lastTapMs = -1;
		int64_t outputUntilMs = -1;
		uint64_t syncSerial = static_cast<uint64_t>(-1);
	} thumbrestTripleTap[2];
	...
	void UpdateThumbrestTripleTap(int hand, bool raw, bool active, const char* handName);
